...
class LinkList extends Component {
  ...
  _subscribeToNewLinks = () => {
    this.props.allLinksQuery.subscribeToMore({
      document: SUBSCRIBE_NEW_LINK,
      updateQuery: (previous, { subscriptionData: { data } }) => {
        const newAllLinks = [data.Link.node, ...previous.allLinks];
        const result = {
          ...previous,
  ...
  _subscribeToNewVotes = () => {
    this.props.allLinksQuery.subscribeToMore({
      document: SUBSCRIBE_NEW_VOTE,
      updateQuery: (previous, { subscriptionData: { data } }) => {
        const votedLinkIndex = previous.allLinks.findIndex(
          link => link.id === data.Vote.node.link.id
        );
        const link = data.Vote.node.link;
